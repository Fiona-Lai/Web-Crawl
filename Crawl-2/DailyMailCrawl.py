...
driver.get(f'https://www.dailymail.co.uk/home/search.html?offset=0&size=50&sel=site&searchPhrase={keyword}&sort=recent&type=article&type=video&type=permabox&days=all')
...
link = html.xpath('//div[@class="sch-results"]/div/a/@href')
time.sleep(10)
#关闭浏览器的
driver.quit()
headers = {
    'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36',
    'referer': 'https://www.dailymail.co.uk/news/index.html',
...
for i in range(0,10):
    # link_jia = ""
    link_jia = 'https://www.dailymail.co.uk/' + link[i]
    s_response = requests.get(url = link_jia,headers=headers)
    s_html = etree.HTML(s_response.content)
    print(link_jia)
    title = s_html.xpath('//div[@class="alpha"]/div/h2/text()')[0]
    print(title)
    article = s_html.xpath('//div[@itemprop="articleBody"]/p/text()')
    print(article)
    time = s_html.xpath('//p/span[1]/time/text()')[0]
    print(time)
    print("-"*20)